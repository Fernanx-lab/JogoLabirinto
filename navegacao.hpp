...
// Implementação inteira no header (inline/static) para seguir o padrão do projeto.

static inline bool dentro_limites(int l, int c, int altura, int largura) {
    return l >= 0 && l < altura && c >= 0 && c < largura;
}

// Retorna o caminho (lista de pares linha,coluna) do início até o destino.
// Se não houver caminho, retorna vetor vazio.
static inline std::vector<std::pair<int,int>> bfs_obter_caminho(const Mapa& mapa,
...
// Faz o personagem seguir o caminho mais curto até a célula destino usando BFS.
// Retorna true se o personagem foi movimentado até o destino; false caso contrário.
// Se out_path não for nullptr, o caminho encontrado será preenchido nele (inclui início e destino).
static inline bool personagem_navegar_para(Mapa& mapa, Personagem* p, int goalLinha, int goalColuna,
